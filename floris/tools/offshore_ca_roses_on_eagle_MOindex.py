...
def main():
    ...
    for i in range(len(sites['latitude'])):
        ...
        for filename in glob.glob(fo_path): 
            ...
            ##############################################################################################
            # Limit the search through coordinates for only those within 0.25 degrees of target lat/lon
            diff_df = np.abs(coord_df[0] - lat_target) + np.abs(coord_df[1] - lon_target)
            diff_sub = diff_df.loc[diff_df < 0.5].to_frame()
            diff_ind = diff_sub.index # Get indices

        

            # Loop through indices, calculate greatest circle distance between modeled lat lon and target lat/lon
            for m in diff_ind:
                diff_sub.loc[m, 'dist'] = gp.great_circle((lat_target,lon_target),(coord_df.loc[m,0],coord_df.loc[m,1])).km
            
            coord_loc = diff_sub['dist'].idxmin()
            ######################################################################################
